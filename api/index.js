...
app.post('/api/send-offer', async (req, res) => {
  ...
  try {
    // Generate the entire document structure through native JS objects
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun(`This Agreement is effective as of `),
              new TextRun({ text: `${startDate}`, bold: true }),
              new TextRun(` and is entered into between FanFame Media and `),
              new TextRun({ text: `${name}`, bold: true }),
              new TextRun(`.`),
            ],
          }),
          new Paragraph({ text: "" }),
          new Paragraph({ text: "Both parties agree to the following terms and conditions:" }),
          new Paragraph({ text: "" }),
          ...
          new Paragraph({
            children: [
              new TextRun(`The Assistant shall receive a fixed salary of `),
              new TextRun({ text: `₹${salary}`, bold: true }),
              new TextRun(` per month. Any future salary increases, bonuses, or additional compensation shall be determined solely at the discretion of the Company based on performance, reliability, attendance, and overall contribution.`),
            ],
          }),
          new Paragraph({ text: "" }),
    ...
    // Compile into a Node buffer stream
    const updatedDocBuffer = await Packer.toBuffer(doc);

    // Setup Nodemailer Mail Delivery Engine
